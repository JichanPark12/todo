...
const TodoButton = ({ size, color, children, ...props }: Props) => {
  return (
    ...
    >
      <div className="w-full h-full absolute left-[2.15px] top-[2.5px] rounded-3xl bg-slate-900 border-2 border-slate-900" />
      <div
        className={`w-full h-full absolute left-[-1.5px] top-[-1.5px] rounded-3xl bg-${color} border-2 border-slate-900 flex items-center justify-center`}
      >
        {children}
      </div>
    </button>
